...
{	
	...
	{
		/*
		Magazine ingredient2;
		Class.CastTo(ingredient2, ingredients[1]);
		Magazine result;
		Class.CastTo(result, results.Get(0));
		
		int num_of_rounds = ingredient2.GetAmmoCount();
		result.SetAmmoCount(num_of_rounds);
		*/
		PaintItem.Paint(ingredients[0], ingredients[1], "Mag_AKM_Drum75Rnd", player, specialty_weight);
	}
};
