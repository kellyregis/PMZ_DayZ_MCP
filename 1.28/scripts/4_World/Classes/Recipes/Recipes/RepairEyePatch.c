...
{	
	...
	{
		ItemBase ingrEP = ingredients[0];
		ItemBase ingrRag = ingredients[1];
		
		if(ingrRag.GetHealthLevel() >= ingrEP.GetHealthLevel())
			return false;
	...
	override void Do(ItemBase ingredients[], PlayerBase player,array<ItemBase> results, float specialty_weight)//gets called upon recipe's completion
	{
		...
		ingrEP.SetHealth01("","",ingrRag.GetHealth01()); // set the health of the eye patch as the rag used to repair it.
	}
};