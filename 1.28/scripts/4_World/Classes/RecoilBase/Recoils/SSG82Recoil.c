class SSG82Recoil: RecoilBase
{
	override void Init()
	{
		...
		vector point_3;
		vector point_4;
		point_1[0] = m_Player.GetRandomGeneratorSyncManager().GetRandomInRange(RandomGeneratorSyncUsage.RGSRecoil,0.5,0.75); point_1[1] = 1.75; point_1[2] = 0;
		point_2[0] = m_Player.GetRandomGeneratorSyncManager().GetRandomInRange(RandomGeneratorSyncUsage.RGSRecoil,-0.25,0.25); point_2[1] = 1.0; point_2[2] = 0;
		...
		m_MouseOffsetRelativeTime = 0.0725;//[0..1] a time it takes to move the mouse the required distance relative to the reload time of the weapon(firing mode)
